...
export const PortfolioPositionsService = {
  getPortfolioValues: async (): Promise<Portfolio[]> => {
    return PortfolioPositionsService.getPortfolioValues();
  },
  ...
  getPortfolioValuesByDateRange: async (startDate: Date, endDate: Date): Promise<Portfolio[]> => {
    ...
    const portfolioHistory = await PortfolioPositionsService.getPortfolioValuesByDateRange(
      1,
      dateRange,
    );
  ...
  getLatestPortfolioValue: async (): Promise<Portfolio | null> => {
    return PortfolioPositionsService.getLatestPortfolioValue();
  },
};
